...
namespace Common.Enums
{
    public static class EnumExtensions
    {
        ...
        public static T ToEnum<T>(this string value) where T : struct, IComparable, IFormattable, IConvertible
        {
            ...
            {
                return ToFlagEnum<T>(value.Split(',').Where(t => !string.IsNullOrWhiteSpace(t) && Enum.IsDefined(type, t.Trim())).Select(t => Enum.Parse(type, value, true)).ToArray());
            }

            try
            {
                return Enum.IsDefined(type, value) ? (T)Enum.Parse(type, value) : (T)Enum.Parse(type, value, true);
            }
            catch
            {
                return default;
        ...
        public static Dictionary<string, Dictionary<string, string>> GetEnums(this Assembly ass)
        {
            ...
            foreach (Type type in ass.GetTypes().Where(t => t.IsEnum))
            {
                ...
                {
                    dic[enumName].Add(item.ToString(), ((Enum)item).GetDescription());
                }
            }
            return dic;
        }
